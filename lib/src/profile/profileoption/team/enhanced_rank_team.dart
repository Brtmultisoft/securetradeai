...
class EnhancedRankTeamState extends State<EnhancedRankTeam> {
  ...
  int _indexFromApiRank(String rank) {
    ...
    if (idx == -1 && normalized == 'grand legend') {
      idx = names.indexOf('Crypto Legend');
    }
  ...
  Future<void> _loadUserRank() async {
    try {
      ...
      // ignore errors, default stays No Rank
    }
  }
  ...
  Widget _getRank() {
    ...
    // Determine user's current rank from API (mapped index)
    int userRank = _apiUserRankIndex;

    // We don't compute progress/requirements here; API provides it
